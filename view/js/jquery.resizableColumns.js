...
(function($, window) {
  var ResizableColumns, parseWidth, pointerX, setWidth,setClass;
  parseWidth = function(node) {
    return parseInt(node.className.split(' ').pop().replace('col-md-',''));
  };
  ...
  setClass=function (node,newClass,oldClass){
    node.removeClass(function(i, j) {
      var toremove='';
      if(j.match(/col-md-\w+/gi)) {
        toremove = j.match(/col-md-\w+/gi).join(" ");
      }
      return toremove;
    });

    node.addClass('col-md-'+newClass);
  };
  pointerX = function(e) {
  ...
  ResizableColumns = (function() {
    ...
    ResizableColumns.prototype.pointerdown = function(e) {
      ...
      $ownerDocument.on('mousemove.rc touchmove.rc', (function(_this) {
        return function(e) {
          var difference,colDiff;
          difference = (pointerX(e) - startPosition) / _this.$table.width() * 100;
          colDiff=parseInt(difference/8.33);
          newWidths.left=widths.left+colDiff;
          newWidths.right=widths.right-colDiff;
          if(newWidths.left>=0&&newWidths.right>=0) {
            setClass($leftColumn, newWidths.left,widths.left);
            setClass($rightColumn, newWidths.right,widths.right);
          }
